...
std::vector<MetaInfo::Property> MetaInfo::extractProperties(const Cursor& cursor) const {
    ...
    auto&&                   properties         = ParserUtils::split(propertyList, ",");
    static const std::string white_space_string = " \t\r\n";

    for (auto& property_item : properties) {
        auto&& item_details = ParserUtils::split(property_item, ":");
        auto&& temp_string  = ParserUtils::trim(item_details[0], white_space_string);
        if (temp_string.empty()) {
            continue;
        }
        ret_list.emplace_back(temp_string,
                              item_details.size() > 1 ? ParserUtils::trim(item_details[1], white_space_string) : "");
    }
    return ret_list;
}
